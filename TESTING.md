...
- Background video autoplays permanently muted. On the first visit, BEGIN remains silent through the poem and final image; pressing ENTER starts the Muhammad Ali training ambience from a random point and fades it to a restrained 9% desktop output or quieter 5% mobile output. On later visits, CONTINUE primes that ambience silently behind the loading veil and fades it in with the menu. A shared Web Audio context and gain layer provide the same fade behaviour on mobile browsers that restrict direct media-element volume changes. SOUND OFF fades to mute while playback continues, SOUND ON fades the advancing playback back in, and ambience continues into rooms without an outline on its control.
...
- Entering Documentary fades the ambience to mute beneath the loading veil while its playback position continues silently. Leaving Documentary fades it back from that position unless the visitor selected SOUND OFF.
...
- On mobile, the first direct touch unlocks and warms the shared Web Audio path before delegated controls request their selection or confirmation feedback. The mobile mix raises the short SFX above its deliberately quieter ambience bed.
